...
def get_user_by_name(contributor_name: str) -> Dict[str, Any]:
    """
    Makes a POST request to get a user by name.

    Args:
        contributor_name (str): The username of the contributor.
...
def parse_get_user_by_name_response(response):
    """
    ...
    if response.status_code == 200:
        try:
            data = response.json()
            user_data = data.get('data', {}).get('getUserByName', {})
